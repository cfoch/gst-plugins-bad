...
do_seek (GstBaseSrc * bsrc, GstSegment * segment)
{
  ...
  src->index = src->fake_index = DEFAULT_START_INDEX;
  return TRUE;
}
...
gst_imagesequencesrc_set_location_from_file (GstImageSequenceSrc * src)
{
  ...
  filenames_strv = (gchar **) g_ptr_array_free (filenames, FALSE);

  g_object_set (src, "filenames-list", filenames_strv,
...
gst_imagesequencesrc_parse_location (GstImageSequenceSrc * src)
{
  ...
  src->index = DEFAULT_START_INDEX;
  src->len = g_list_length (src->filenames);
}
...
gst_imagesequencesrc_set_duration (GstImageSequenceSrc * src)
{
  ...
  src->duration =
      gst_util_uint64_scale (GST_SECOND * src->len, src->fps_d, src->fps_n);
}
  ...
  switch (property_id) {
    ...
    case PROP_FILENAMES:
    {
      ...
      if (filenames)
        ...
        GST_DEBUG_OBJECT (src, "Set (filenames) property. NULL.");
...
gst_imagesequencesrc_create (GstPushSrc * bsrc, GstBuffer ** buffer)
{
  ...
  if (src->index == DEFAULT_START_INDEX)
  ...
  GST_BUFFER_PTS (buf) = GST_BUFFER_DTS (buf) =
      src->fake_index * buffer_duration;
